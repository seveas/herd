...
func init() {
	katyusha.RegisterProvider("consul", newProvider, magicProvider)
}
...
func (p *consulProvider) Load(ctx context.Context, lm katyusha.LoadingMessage) (katyusha.Hosts, error) {
...
func (p *consulProvider) loadDatacenter(dc string) (katyusha.Hosts, error) {
	...
	for i, node := range catalognodes {
		...
		hosts[i] = katyusha.NewHost(node.Node, ap[0], katyusha.HostAttributes{"datacenter": node.Datacenter})
	}
