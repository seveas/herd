...
func init() {
	katyusha.RegisterProvider("consul", newConsulProvider, consulProviderMagic)
}
...
func newConsulProvider(name string) katyusha.HostProvider {
...
func consulProviderMagic(r *katyusha.Registry) {
...
func (p *consulProvider) Equivalent(o katyusha.HostProvider) bool {
...
func (p *consulProvider) Load(ctx context.Context, mc chan katyusha.CacheMessage) (katyusha.Hosts, error) {
...
func (p *consulProvider) loadDatacenter(conf *consul.Config, dc string) (katyusha.Hosts, error) {
	...
	for i, node := range catalognodes {
		...
		hosts[i] = katyusha.NewHost(node.Node, katyusha.HostAttributes{"datacenter": node.Datacenter})
	}
