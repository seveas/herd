...
func TestFindingProvider(t *testing.T) {
	...
	if p.config.Command != filepath.Join(testdata, "bin", "katyusha-provider-fake") {
		t.Errorf("Provider was not found by name automatically")
	}
	p = newPlugin("fake2").(*pluginProvider)
	if p.config.Command != filepath.Join(testdata, "bin", "katyusha-provider-fake2.exe") {
...
func TestPluginConnection(t *testing.T) {
	...
	for _, test := range testcases {
		t.Run(test.mode, func(t *testing.T) {
			...
			logrus.AddHook(hook)
			if p.config.Command != filepath.Join(testdata, "bin", "katyusha-provider-ci") {
				t.Errorf("Provider was not found by name automatically")
			}
			v := viper.New()
