...
func init() {
	katyusha.RegisterProvider("google", newProvider, nil)
}
...
func (p *googleProvider) Equivalent(o katyusha.HostProvider) bool {
	op := o.(*googleProvider)
...
func (p *googleProvider) Load(ctx context.Context, lm katyusha.LoadingMessage) (hosts katyusha.Hosts, err error) {
	lm(p.name, false, nil)
...
func (p *googleProvider) loadZone(zone string) (katyusha.Hosts, error) {
	ctx := context.Background()
	...
	for {
		...
		ret = append(ret, katyusha.NewHost(name, *inst.NetworkInterfaces[0].NetworkIP, attrs))
	}
